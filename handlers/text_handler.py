...
def text_message_handler(message):
    ...
    if User.get_or_none(User.user_id == user_id) is None:
        ...
        if message.text.lower() == 'погода':
            logger.info(f'{username} with id {user_id} requests weather!')
            ...
            logger.info(f'{username} with id {user_id} requests forecast!')
            bot.set_state(message.from_user.id, MyStates.get_city_forecast, message.chat.id)
            bot.send_message(message.chat.id, 'Введите населенный пункт')

        elif message.text.lower() == 'помощь':
            logger.info(f'{username} with id {user_id} requests help!')
